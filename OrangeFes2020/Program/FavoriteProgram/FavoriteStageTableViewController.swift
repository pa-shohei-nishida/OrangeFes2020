//
//  FavoriteStageTableViewController.swift
//  SchoolFestivalNavi
//
...
class FavoriteStageTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        celectedFestivalID = userDefaults.string(forKey: "celectedFestivalID") ?? ""
        let UDColorTestProgramme = userDefaults.array(forKey: "UDProgrammeKey,\("Stage"),\(celectedFestivalID)") as? [String] ?? []
    ...
    @IBAction func favoriteButtonPressed(_ sender: UIButton) {
        ...
        var getUDProgrammeKey:[String] = userDefaults.array(forKey: "UDProgrammeKey,\("Stage"),\(celectedFestivalID)") as? [String] ?? []
        ...
        userDefaults.set(getUDProgrammeKey, forKey: "UDProgrammeKey,\("Stage"),\(celectedFestivalID)")
    }
    ...
    func getFavoriteProgramme(){
        ...
        UDFavoriteProgrammeKey = userDefaults.array(forKey: "UDProgrammeKey,\("Stage"),\(celectedFestivalID)") as? [String] ?? []
        //bugあり
        if UDFavoriteProgrammeKey == [] {
            ...
            for item in UDFavoriteProgrammeKey {
                ref.child("学園祭一覧").child(celectedFestivalID).child("Programme").child("Stage").child(item).observeSingleEvent(of: .value, with: { (snapshot) in
...
extension FavoriteStageTableViewController: IndicatorInfoProvider {
